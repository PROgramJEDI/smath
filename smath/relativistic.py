...
def relfunc(f, rel, x_start=-20, x_end=20, accuracy=10, dx=10**(-3)):
	...
	# prepare the two groups to be integrated seperatly.
	a_negetives, a_positives = a_range[a_range < 0], a_range[a_range > 0] 
	a_range = np.concatenate((a_negetives, a_positives))

	# the continous integral of the two groups.
	negetive_x_arc_length = integral(f_arc_length(a_negetives), a_negetives, dx=dx)	
	positive_x_arc_length = integral(f_arc_length(a_positives), a_positives, dx=dx, initial=0)

	# reduce to the same shape of the axis of the negetives integration.
	a_range = a_range[1:]
	...
	# the orthagonal linear function to the tangent in a specific point.
